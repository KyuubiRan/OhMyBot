...
internal static class GatewayCommandParser
{
    ...
    public static bool IsCommand(string text, IReadOnlyCollection<string> prefixes)
    {
        return TryFindPrefix(text.TrimStart(), NormalizePrefixes(prefixes), out _);
    }

    public static (string Command, string[] Args) Parse(
        string text,
        IReadOnlyCollection<string> prefixes,
        bool stripBotMention = false)
    {
        ...
        var command = parts[0][prefix.Length..];
        if (stripBotMention)
        {
            command = command.Split('@', 2)[0];
        }

        return (command.ToLowerInvariant(), parts.Skip(1).ToArray());
    }
