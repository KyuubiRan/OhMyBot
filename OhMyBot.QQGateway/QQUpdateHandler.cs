    ...
    ILogger<QQUpdateHandler> logger)
{
    ...
    private readonly SemaphoreSlim _processingGate =
        new(Math.Max(1, options.Value.MaxConcurrentMessages));
    ...
    public void Handle(EventBase evt)
    {
        ...
        // 并发上限：群消息洪水下宁可丢弃也不无限开任务堆积（每条都带 gRPC 调用）。
        if (!_processingGate.Wait(0))
        {
            logger.LogWarning(
                "QQ 消息处理并发已达上限 {Limit}，丢弃消息 message_id={MessageId}。",
    ...
    private async Task ProcessAsync(MessageEvent message)
    {
        try
        {
            ...
            // 放在菜单选择判断之前：私聊里发纯数字但没有活动菜单时，这条消息同样该被记录。
