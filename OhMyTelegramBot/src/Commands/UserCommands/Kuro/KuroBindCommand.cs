...
public sealed class KuroBindCommand(KuroAccountService kuroAccountService, BotActionManager actionManager) : ICommand
{
    ...
    public async Task OnReceiveCommand(ITelegramBotClient botClient, Message message, long chatId, long senderId, string[] args)
    {
        ...
        try
        {
            var payload = new KuroBindPayload(kuid, token, devCode, distinctId, ipAddress);
            var preview = await kuroAccountService.GetBindPreviewAsync(senderId.ToString(), SoftwareType.Telegram, payload);
            var text = """
                       用户名：{0}
                       UID：{1}
                       """.Fmt(preview.UserName, preview.UserId);
                    ...
                    [
                        ...
                        InlineKeyboardButton.WithCallbackData(
                            "取消绑定", await actionManager.PutActionAsync("kuro_bind", chatId, senderId,
                                                                       new KuroBindActionData(false, kuid, token, devCode, distinctId, ipAddress)))
                    ]
                ]
            });
        }
        ...
        {
            await botClient.EditMessageText(chatId, msg.Id, e.GetBaseException().Message);
        }
    }
}