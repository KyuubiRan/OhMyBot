...
public static class RemoteConsoleClient
{
    public static async Task<bool> TryRunAsync(IConfiguration configuration, CancellationToken cancellationToken = default)
    {
        ...
        // 远程控制台与 Core 共用同一份 Host 配置，令牌直接读 Grpc:AccessToken；
        // 没配则回落到 Core 启动时写下的共享令牌文件。
        using var channel = GrpcChannel.ForAddress(coreAddress, new GrpcChannelOptions
        {
            HttpHandler = handler,
            Credentials = GrpcAccessCredentials.Create(GrpcSharedToken.Resolve(configuration["Grpc:AccessToken"])),
            UnsafeUseInsecureChannelCallCredentials = true
        });
        var client = new CommandRouter.CommandRouterClient(channel);

        using var probeCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        // 冷启动时这一步要现加载 gRPC 栈并完成 HTTP/2 握手，ARM 小主机上 800ms 会偶发不够，
        // 表现为「明明 Core 在跑却报连不上」。放宽到 3s：连不上时多等两秒，远好过误判。
