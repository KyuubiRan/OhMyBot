    ...
    IOptions<CallbackActionOptions> options)
{
    ...
    /// <summary>消费（删除）一次性回调动作，防止按钮被重复点击造成重放。</summary>
    public Task RemoveAsync(string hash, CancellationToken cancellationToken = default)
    {
        return cache.RemoveAsync(GetKey(hash), cancellationToken);
    }
